...
func (this *ReviewController) List() {
	...
	list, count := new(models.Task).GetList(page, this.pageSize, filter...)
	envList := make(map[int]*models.Env)
	for k, v := range list {
		if _, ok := envList[v.PubEnvId]; !ok {
			envList[v.PubEnvId], _ = new(models.EnvServer).GetEnv(v.PubEnvId)
		}
...
func (this *ReviewController) Review() {
	...
	if this.isPost() {
		...
		err := new(models.Task).ReviewTask(id, this.userId, status, message)
		this.checkError(err)
		new(models.Action).Add("review_task", this.auth.GetUserName(), "task", id, this.GetString("status"))
		this.redirect(beego.URLFor("ReviewController.List"))
	}

	task, err := new(models.Task).GetTask(id)
	this.checkError(err)
...
func (this *ReviewController) Detail() {
	...
	env, err := new(models.EnvServer).GetEnv(task.PubEnvId)
	this.checkError(err)
	review, err := new(models.Task).GetReviewInfo(id)
	if err != nil {
