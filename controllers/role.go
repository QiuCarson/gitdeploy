...
func (this *RoleController) List() {
	roleList, err := new(models.Role).GetAllRoles()
	this.checkError(err)
	for k, role := range roleList {
		roleList[k].UserList, _ = new(models.User).GetUserListByRoleId(role.Id)
	}
...
func (this *RoleController) Add() {
	if this.isPost() {
		...
		err := new(models.Role).AddRole(role)
		this.checkError(err)
...
func (this *RoleController) Edit() {
	id, _ := this.GetInt("id")
	role, err := new(models.Role).GetRole(id)
	this.checkError(err)

	if this.isPost() {
		role.RoleName = this.GetString("role_name")
		role.Description = this.GetString("description")
		err := new(models.Role).UpdateRole(role, "RoleName", "Description")
		this.checkError(err)
		this.redirect(beego.URLFor("RoleController.List"))
	}
...
func (this *RoleController) Del() {
	...
	err := new(models.Role).DeleteRole(id)
	this.checkError(err)
...
func (this *RoleController) Perm() {
	...
	if this.isPost() {
		...
		err := new(models.Role).UpdateRole(role, "ProjectIds")
		this.checkError(err)
		err = new(models.Role).SetPerm(role.Id, perms)
		this.checkError(err)
		this.redirect(beego.URLFor("RoleController.List"))
	}

	projectList, _ := new(models.Project).GetAllProject()
	permList := new(models.SystemService).GetPermList()
