...
func (this *TaskController) List() {
	...
	list, count := tasks.GetList(page, this.pageSize, filter...)
	projectList, _ := projects.GetAllProject()
	...
	if this.isPost() {
		...
		if verType == 2 {
			...
			if libs.VerCompare(startVer, endVer) != -1 {
				...
				repo, _ := repositorys.GetRepoByProjectId(projectId)
		...
		// 构建任务
		go tasks.BuildTask(task)

		actions.Add("create_task", this.auth.GetUserName(), "task", task.Id, "")
...
func (this *TaskController) GetTags() {
	...
	list, err := repositorys.GetTags(projectId, 10)
	this.checkError(err)
...
func (this *TaskController) Detail() {
	...
	task, err := new(models.Task).GetTask(taskId)
	this.checkError(err)
	env, err := new(models.EnvServer).GetEnv(task.PubEnvId)
	this.checkError(err)
	review, err := new(models.Task).GetReviewInfo(taskId)
	if err != nil {
		review = new(models.TaskReview)
	}
...
func (this *TaskController) Publish() {
	...
	serverList, err := new(models.Env).GetEnvServers(task.PubEnvId)
	this.checkError(err)
	env, err := new(models.EnvServer).GetEnv(task.PubEnvId)
	this.checkError(err)
...
func (this *TaskController) StartPub() {
	...
	err := new(models.Deploy).DeployTask(taskId)
	this.checkError(err)

	new(models.Action).Add("pub_task", this.auth.GetUserName(), "task", taskId, "")
...
func (this *TaskController) Del() {
	...
	err := new(models.Task).DeleteTask(taskId)
	this.checkError(err)

	new(models.Action).Add("del_task", this.auth.GetUserName(), "task", taskId, "")
