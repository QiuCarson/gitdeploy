...
func (this *UserController) List() {
	...
	count, _ := new(models.User).GetTotal()
	users, _ := new(models.User).GetUserList(page, this.pageSize, true)
...
func (this *UserController) Add() {
	if this.isPost() {
		...
		user, err := new(models.User).AddUser(username, email, password1, sex)
		this.checkError(err)
		...
		new(models.User).UpdateUserRoles(user.Id, roleIds)

		this.redirect(beego.URLFor("UserController.List"))
	}

	roleList, _ := new(models.Role).GetAllRoles()
...
func (this *UserController) Edit() {
	id, _ := this.GetInt("id")
	user, err := new(models.User).GetUser(id, true)
	this.checkError(err)
	...
	if this.isPost() {
		...
		user.Status = status
		user.Email = email
		new(models.User).UpdateUser(user, "Sex", "Status", "Email")

		if password1 != "" {
			new(models.User).ModifyPassword(user.Id, password1)
		}
...
func (this *UserController) Del() {
	...
	err := new(models.User).DeleteUser(id)
	this.checkError(err)
