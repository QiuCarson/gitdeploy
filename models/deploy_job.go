...
func NewDeployJob(t *Task) *DeployJob {
	job := new(DeployJob)
	job.task = t
	job.init()
	return job
...
func (this *DeployJob) init() {
	// 初始化项目信息
	this.project, _ = new(Project).GetProject(this.task.ProjectId)
	this.agent, _ = new(Server).GetServer(this.project.AgentId)
	// 环境信息
	this.env, _ = new(EnvServer).GetEnv(this.task.PubEnvId)
	// 任务在跳板机的目录
