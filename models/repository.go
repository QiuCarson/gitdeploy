...
// 返回一个仓库对象
func (this *Repository) GetRepoByProjectId(projectId int) (*Repository, error) {

	project, err := new(Project).GetProject(projectId)
	if err != nil {
		return nil, err
	}
...
// 获取某项目代码库的标签列表
func (this *Repository) GetTags(projectId int, limit int) ([]string, error) {
	repo, err := this.GetRepoByProjectId(projectId)
	if err != nil {
		return nil, err
	}
	repo.Pull()
	list, err := repo.GetTag()
	if err != nil {
		return nil, err
	}
	if len(list) > limit {
...
// 获取tag列表
func (repo *Repository) GetTag() ([]string, error) {
